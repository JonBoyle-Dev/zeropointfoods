...
└── src/
    ...
    ├── components/{onboarding,foods,log,today,recipes,library,profiles,layout,reports,common}/
    └── pages/             # route-level screens (ProfileSelect, Today, Log, Foods, Recipes, Library, Reports)
```
...
- **The weekly bank display is per-day green/red, not an abstract coin-fill bar.** `DayNav`'s chips color by that day's stored `daily_summary.is_over_budget` (`useWeekOverBudgetStatus` in `src/hooks/useDailySummary.ts`) — green if on/under, red if over, and *left neutral* (not assumed green) for a day with no `daily_summary` row yet, since there's no verdict without data. `WeeklyBankStrip` was simplified to just the plain running total (`weekly_cycles.weekly_bank_current`) — the old proportional coin-bar visualization is gone.
- **Reports (`src/pages/ReportsPage.tsx`, `src/hooks/useReports.ts`) is read-only aggregation, no new state.** `useWeeklyTrend` sums existing `daily_summary` rows into 5 week-buckets (oldest → current, left → right) rather than introducing its own per-week table — a week with zero rows just renders as an empty/near-zero bar. `useCategoryBreakdown` sums `food_entries.points_used` grouped by `foods.category` for one date range (client-side aggregation — Supabase's JS client has no `GROUP BY`). The trend bars reuse the same green/under, red/over color convention as `DayNav`, just at week granularity instead of day.
